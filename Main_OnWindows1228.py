...
class TeamPage(tk.Frame):
    ...
    def createWidgets(self):
        ...
        def click_team_button(team_name):
            ...
            self.Label= tk.Label(self.scrollableF, bg="bisque2")
            ...
            for i in range(len(players)):
                ...
                self.PInfoLabel= tk.Label(self.scrollableF, bg="bisque2")
            ...
            for game in games[1:-2]:
                # print(game)
                self.GInfoLabel= tk.Label(self.scrollableF, bg="bisque2")
...
class PersonalPage(tk.Frame):
    ...
    def modify(self):
        ...
        self.F2_canvas = tk.Canvas(self, width = 500, height = 125 + 190 * len(user_info[4]), bg = "old lace", highlightthickness = 0)  #height調整canvas的長度，要手動調（或寫def）
        self.F2_canvas.pack(side = "top",fill = "both", expand = True)
        # 要建立frame，透過create_widget放在canvas上面才能滾動
        self.F2 = tk.Frame(self.F2_canvas, bg = "old lace", width = 500, height = 125 + 190 * len(user_info[4]))
        self.F2.pack(side = "top", fill = "both" ,expand = True)
        self.F2_canvas.create_window((0,0), window = self.F2, anchor = "nw") 
        # 滾動條
        self.gameBar = tk.Scrollbar(self.F2_canvas, orient = "vertical", command = self.F2_canvas.yview)
